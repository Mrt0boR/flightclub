...
func Run(args []string) int {
	...
	var (
		...
		notifierList = flags.String("notify", "console", "comma-separated: console, desktop, webhook, discord")
		webhookURL   = flags.String("webhook-url", os.Getenv("FLIGHTTRACK_WEBHOOK"), "https URL to POST events to")
		discordURL   = flags.String("discord-url", os.Getenv("FLIGHTTRACK_DISCORD"), "Discord channel webhook URL")
	...
	notifiers, err := buildNotifiers(*notifierList, *webhookURL, *discordURL)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 2
	}
...
func buildNotifiers(notifierList, webhookURL, discordURL string) (*notify.Set, error) {
	...
	for _, name := range strings.Split(notifierList, ",") {
		...
		case "webhook":
			...
			notifiers.Add(hook)
		case "discord":
			d, err := notify.NewDiscord(discordURL)
			if err != nil {
				return nil, err
			}
			notifiers.Add(d)
